...
type SignInStep = "email" | Exclude<McpGuestRequestStatus, "not_submitted">;

interface McpGuestSignInFlowProps {
  initialWorkEmail?: string;
}

export default function McpGuestSignInFlow({
  initialWorkEmail = "",
}: McpGuestSignInFlowProps) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (result.status === "not_submitted") {
        router.replace(
          `${MCP_GUEST_ENTRY_PATH}?email=${encodeURIComponent(trimmedEmail)}`
        );
        return;
      }
