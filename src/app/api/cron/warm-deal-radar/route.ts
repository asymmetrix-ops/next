...
import { warmDealRadarCache } from "@/lib/dealRadarServer";
...
async function warmDealRadar(
  ...
  try {
    ...
    if (!token) {
      console.error("[CRON] warm-deal-radar: Failed to authenticate with Xano", {
    ...
    const ok = await warmDealRadarCache(token);
    if (!ok) {
      console.error("[CRON] warm-deal-radar: Redis not configured");
      return;
    }

    const totalMs = Math.round(performance.now() - start);
    console.log(
      `[CRON] warm-deal-radar cached key=${DEAL_RADAR_CACHE_KEY} totalMs=${totalMs}`
    );
  } catch (error) {
    console.error("[CRON] warm-deal-radar failed:", error);
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!force && !ok) {
      console.log(`[CRON] Skipping warm-deal-radar (London hour=${londonHour})`);
      return NextResponse.json({
        success: true,
        skipped: true,
  ...
  runInBackground(warmDealRadar(bearerFromReq));

  return NextResponse.json({
    success: true,
    started: true,
    cachedKey: DEAL_RADAR_CACHE_KEY,
    limit: 25,
    offset: 0,
  });
}
