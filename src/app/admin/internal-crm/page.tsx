...
import { authService as mainAuthService } from "@/lib/auth";
import {
  authService,
  isAdminUser,
  isTokenExpired,
...
const INTERNAL_CRM_PATH = "/admin/internal-crm";
import {
...
function InternalCrmPageInner() {
  ...
  const loginRedirectPath = useMemo(() => {
    ...
    const redirect = `${INTERNAL_CRM_PATH}${query ? `?${query}` : ""}`;
    return `/login?redirect=${encodeURIComponent(redirect)}`;
  }, [searchParams]);

  const ensureContributorSession = useCallback(async (): Promise<string | null> => {
    const mainToken = mainAuthService.getToken();
    if (!mainToken || isTokenExpired(mainToken)) {
      return null;
    }
  ...
  }, [authLoading, isAdmin, isAuthenticated, loginRedirectPath, router]);
  ...
  const fetchData = useCallback(async () => {
    ...
    const token = await ensureContributorSession();
    if (!token) {
      setError("Unable to initialize contributor CRM session. Please refresh the page.");
      setLoading(false);
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      if (
        ...
        const retryToken = await ensureContributorSession();
        if (retryToken) {
          try {
            const res = await getFinMetricsCompanies(retryToken, {
              search: appliedSearch || null,
              status_filter: appliedStatusFilter || null,
              page,
              per_page: perPage,
  ...
  const exportCsv = useCallback(async () => {
    const token = await ensureContributorSession();
    if (!token) {
      toast.error("Unable to initialize contributor CRM session.");
      return;
    }
  ...
  }, [appliedSearch, appliedStatusFilter, ensureContributorSession]);
  ...
  const saveWorkflow = useCallback(
    ...
    [ensureContributorSession, update]
  );
  ...
  useEffect(() => {
    ...
    if (requestedReviewCompanyId == null) {
      ...
      router.replace(INTERNAL_CRM_PATH);
      return;
    }
  ...
  if (authLoading || !isAuthenticated || !isAdmin) {
    return (
      <div className="flex justify-center items-center min-h-screen text-sm text-gray-500">
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 p-8 font-mono">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-7">
        <div>
          <Link href="/admin" className="text-xs text-blue-600 hover:underline mb-2 inline-block">
            ← Back to Admin
          </Link>
