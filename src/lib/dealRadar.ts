...
export const COUNTRY_FLAG_ASPECT_RATIO = 4 / 3;

export const INLINE_COUNTRY_FLAG_CLASS =
  "ml-1 inline-block shrink-0 rounded-sm object-cover ring-1 ring-black/10 cursor-default align-middle";

export const getCountryFlagDimensions = (
  size: number = COUNTRY_FLAG_INLINE_SIZE_PX
): { width: number; height: number } => ({
  width: Math.round(size * COUNTRY_FLAG_ASPECT_RATIO),
  height: size,
});
